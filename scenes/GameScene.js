...
export class GameScene extends Phaser.Scene {
  ...
  preload() {
    ...
    this.load.spritesheet("gems", "https://i.ibb.co/SdWmw8n/donut-gems.png", {
  ...
  create() {
    ...
    this.canPick = true;
    this.dragging = false;
    this.bg = this.add.tileSprite(500, 500, 1000, 1000, 'background').setScale(1.0);

    this.score = 0;
    this.drawField();
    this.timeLabel = this.add.text(740, 50, "00:00", { font: "100px Arial", fill: "#fff" })
    this.scoreText = this.add.text(740, 150, "score: 0", { font: "60px Arial", fill: "#fff" });
    ...
    for (let i = 0; i < gameOptions.fieldSize; i++) {
      ...
      for (let j = 0; j < gameOptions.fieldSize; j++) {
        ...
        if (colorToWatch != currentColor || j == gameOptions.fieldSize - 1) {
          if (colorStreak >= 3) {
            if (direction == HORIZONTAL) {
              console.log("Гориз " + colorStreak + startStreak + "Колір " + currentColor);
            }
            else {
              console.log("Вертик " + colorStreak + startStreak + " Колір " + currentColor);
            }
